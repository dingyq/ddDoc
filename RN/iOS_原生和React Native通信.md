...
更多的给JavaScript发送事件的例子，参见`RCTLocationObserver`.
...
请注意，一旦你在IOS中混用2种语言, 你还需要一个额外的桥接头文件，称作`bridging header`，用来导出Objective-C文件给Swift。如果你是通过Xcode菜单中的`File>New File`来创建的Swift文件，Xcode会自动为你创建这个头文件。在这个头文件中，你需要引入`RCTBridgeModule.h`。
...
同样的，你也可以使用`RCT_EXTERN_REMAP_MODULE`和`RCT_EXTERN_REMAP_METHOD`来改变导出模块和方法的JavaScript调用名称。 了解更多信息，请参阅`RCTBridgeModule`.